...
function loadMissions() {
  ...
  MISSIONS.forEach((m, i) => {
    ...
    li.innerHTML = `
      ...
      <button class="mission-btn" onclick="completeMission('${m.id}', '${m.title}')">✔ Fatto</button>
    `;
    list.appendChild(li);
  });
...
function updateMissionUI() {
  MISSIONS.forEach(m => {
    ...
    if (completedMissions.has(m.id)) {
      ...
      li.querySelector("button").outerHTML = `<button class="mission-btn done-btn">✔ Done</button>`;
    }
  });
}

window.completeMission = async function(missionId, missionTitle) {
  ...
  try {
    ...
    if (btn) { btn.textContent = "✔ Fatto"; btn.disabled = false; }
  }
};
