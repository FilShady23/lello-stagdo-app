...
// =============================================
// BADGE PERSONALI (birre + missioni)
// =============================================
const BADGES = [
...
function loadMissions() {
  ...
  onSnapshot(q, snapshot => {
    ...
    snapshot.forEach(d => completedMissions.add(d.data().missionId));
    setTimeout(() => {
      updateMissionUI();
      updateTopbarXP();
      updateBadges();
...
function renderXPLeaderboard(missionScores, beerScores) {
  ...
  sorted.forEach(([name, xp], i) => {
    ...
    const li = document.createElement("li");
    li.className = "leader-item" + (name === currentUser ? " me" : "") + (i === 0 ? " first" : "");
    li.style.animationDelay = (i * 0.05) + "s";
    li.innerHTML = `
...
function updateTopbarXP() {
  ...
  document.getElementById("topbarBadge").textContent = getBadgeIcon(beerCount, completedMissions.size);
}
...
function updateBadges() {
  ...
  BADGES.filter(b => b.type === "beer").forEach(b => {
  ...
  BADGES.filter(b => b.type === "mission").forEach(b => {
...
function initMap() {
  ...
  pubs.forEach(pub => {
    L.marker([pub.lat, pub.lng]).addTo(map).bindPopup(`
      <div class="pub-popup">
        <div class="pub-popup-name">🍺 ${pub.name}</div>
        <div class="pub-popup-xp">Check-in: +5 XP</div>
        <button class="pub-popup-btn" onclick="checkInPub('${pub.name}')">📍 CHECK-IN (+5 XP)</button>
      </div>
    `);
  });
}
...
window.checkInPub = async function(pubName) {
  if (checkedInPubs.has(pubName)) { showToast("📍 Già fatto check-in qui!"); return; }
  try {
    await addDoc(collection(db, "checkins"), {
      user: currentUser, pub: pubName, xp: 5, time: Date.now()
    });
